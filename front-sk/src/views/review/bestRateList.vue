<template>
    <v-container>
        <p class="display-1" 
        style="color:#ff7f00; font-family: 'Sacramento' !important;"><span class="iconify" data-icon="bx:bx-trending-up" data-inline="false">
            </span> Best
            Rate</p>
        <v-container>
            <v-row
            justify="start">
                ...
                <v-container v-if="bestReviews.length==0">
                    <p 
                    class="title">
                    해당 리뷰가 존재하지 않습니다. </p>
                </v-container>
                ...
                v-bind:key="i+10" xs class="ml-5 mr-5" >

                    <v-card id="review-image" :loading="loading_card"
                     class="mx-auto mr-auto" 
                      max-width="350" width="350" 
                   
                     max-height="500" height="500"
                        @click="storeDetail(bestReview.id)">
                        <v-img height="280" :src="bestReview.src">
                            <!--      -->
                        </v-img>
                        <v-card-title
                        style="font-family: 'Noto Serif KR'"
                        >{{bestReview.title}}</v-card-title>
